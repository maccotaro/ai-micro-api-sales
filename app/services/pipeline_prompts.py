...
## コア媒体と商品データの扱い（重要）
本提案のコア掲載媒体は「{core_media}」（本体掲載）です。ただし松竹梅の各プランは、上記「商品・料金データ」に**実在する媒体・商品（media_name・price）を必ず使用**して構成すること。
- media_name を一律「{core_media}」にせず、商品データにある**実在の商品名・価格**を用いること。
- 松竹梅は「グレード・掲載期間・オプションの組み合わせ」で構成と合計金額に**明確な差**をつけること（梅 < 竹 < 松）。
- total_price は商品データの price を積み上げて算出し、**3段階で同額にしない**こと。
- rationale・recommendation_reason・expected_effect 等の**文章中では、matsu/take/ume ではなく「松」「竹」「梅」と日本語表記**すること（例: 「takeプラン」→「竹プラン」）。
...
12. 掲載媒体だけでは解決しない課題（採用管理の煩雑さ／応募者の一元管理／定着率／オペレーション負荷 等）については、ナレッジベース情報（上記KB情報）にある周辺商材・オプション（例: エンポケ等の採用管理ツール、応募代行、定着支援など）を cross_sell として1件以上提案すること。用語ゆれに注意すること（議事録「ATS」＝「採用管理システム」＝「応募者一元管理」＝該当する周辺商材名）。これは松竹梅（掲載媒体の組み合わせ）とは別枠の「気づき」としての追加提案である
13. cross_sell はナレッジベース情報に根拠がある商材のみを挙げ、根拠が無い場合は空配列 [] とすること（商材名の創作は禁止）
14. cross_sell（周辺商材・追加提案）には、コア媒体「{core_media}」の商品・プランを絶対に含めないこと。クロスセルは {core_media} 以外の媒体・周辺商材に限る（{core_media} は松竹梅のコアとして既に提案済みのため）
...
{{
  ...
  "cross_sell": [
    {{
      "issue_id": "対応する課題ID（例: I-2）",
      "product_name": "周辺商材・オプション名（例: エンポケ）",
      "category": "カテゴリ（例: 採用管理/ATS/定着支援/応募代行 等）",
      "addresses": "この商材が解決する課題の説明",
      "rationale": "提案理由（ナレッジベースの根拠に基づく）",
      "source": "根拠となったナレッジベース文書名"
    }}
  ]
}}"""
