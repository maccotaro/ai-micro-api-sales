...
## 出力ルール（体裁のため厳守）
- confused_with を挙げた場合、comparison_rows は **4〜6行必須**（観点別に1行ずつ。空配列にしないこと）。
- psychological_axes は **3〜4個** 作成すること。"""
...
## 出力ルール（体裁のため厳守）
- 戦略軸は **3個** 作成すること（提案①②③に対応）。
- 各軸に `paradigm`（従来の負のイメージ → 新しい捉え方）と `merits`（短い訴求メリットを3〜5個、各8字以内）を必ず付けること。
- `catchcopies` は各軸 **2〜3個**、本文は **40字以内で簡潔・パンチを効かせ**、説明や括弧書きを本文(text)に混ぜないこと（理由は psychology_link へ）。
- タイトル・メリットは短く。長文の段落は避け、スライドに収まる粒度にすること。
...
{{
  "strategy_axes": [
    {{
      "id": "S-1",
      "title": "戦略軸のタイトル(20字以内)",
      "rationale": "この戦略軸を選んだ理由",
      "target_psychology": "対応するターゲット心理",
      "paradigm": {{"old": "従来の負のイメージ(15字以内)", "new": "新しい捉え方・定義(15字以内)"}},
      "merits": ["訴求メリット(8字以内)を3〜5個", "例:屋内勤務", "例:固定シフト", "例:体力不要"],
      "catchcopies": [
        {{
          "text": "キャッチコピー本文(40字以内・記号◎♪可・括弧の説明は入れない)",
          "psychology_link": "なぜ刺さるかの説明(textには含めず別フィールドに)"
        }}
      ]
    }}
  ],
...
## 出力ルール（厳守）
- **1スライドに収まる分量にすること**（最大16行程度、絶対に20行を超えないこと）
- **1行目は必ず `# {page_title}` で始めること**（H1が無いとスライドのヘッダー帯が消える）
- 本文は箇条書き（`- `）・表・引用を基本とし、**段落文の羅列（散文）は禁止**
- 内容は `##` サブ見出しで2〜3ブロックに分け、ページ全体を使う構成にすること
- 箇条書きは6項目以内、各項目は1-2行以内
- テーブルは最大6行×4列程度
- 引用（`>`）は1-3行以内
